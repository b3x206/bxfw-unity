...
// Info : Heavy on some mobile devices due to GrabPass.

Shader "Custom/Blur/Gaussian Blur" 
{
    Properties
    {
        _Size("Blur (_Size)", Range(0, 30)) = 1
        [HideInInspector] _MainTex("Masking Texture", 2D) = "white" {}
        _AdditiveColor("Additive Tint color", Color) = (0, 0, 0, 0)
        _MultiplyColor("Multiply Tint color", Color) = (1, 1, 1, 1)
    }

    // -- Category keyword is used to apply multiple subshader and pass settings in one block.
    Category
    {
