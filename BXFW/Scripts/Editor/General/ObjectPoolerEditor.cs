        ...
        protected virtual void DrawPoolElementCallback(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            foreach (SerializedProperty visibleChild in poolProperty.GetVisibleChildren())
            {
                ...
                if (visibleChild.name == nameof(ObjectPooler.Pool.tag))
                {
                    ...
                    if (EditorGUI.EndChangeCheck() && target.TagExists(visibleChild.stringValue))
                    {
            ...
            dict.Add("m_pools", new KeyValuePair<MatchGUIActionOrder, Action>(MatchGUIActionOrder.OmitAndInvoke, () =>
            {
                ...
                if (GUILayout.Button("+ Add Element", GUILayout.ExpandWidth(true)))
                {
                    ...
                    BasicDropdown.ShowDropdown(guiAreaRect, new Vector2(lastRepaintAddElementRect.width, dropdownStartingHeight), (dropdown) =>
                    {
                        ...
                        bool tagExists = target.TagExists(addElementTagString);
                        using (EditorGUI.DisabledScope scope = new EditorGUI.DisabledScope(tagExists))
                        {
                            ...
                            if ((GUILayout.Button("Add") || hasPressedEnter) && !tagExists)
                            {
                        ...
                        if (tagExists)
                        {
                            EditorGUILayout.HelpBox("Given tag already exists. Please type a different tag.", MessageType.Info);
                        }
